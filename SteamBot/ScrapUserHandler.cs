...
namespace SteamBot
{
    public class ScrapUserHandler : UserHandler
    {
        ...
        int UserMetalAdded, UserScrapAdded, UserRecAdded, UserRefAdded, BotMetalAdded, BotScrapAdded, BotRecAdded, BotRefAdded, InventoryMetal, InventoryScrap, InventoryRec, InventoryRef, InventoryKeys, OverpayNumKeys, ExcessInScrap, PreviousKeys, WhileLoop, InvalidItem = 0;
        ...
        public override void OnTradeReady(bool ready)
        {
            ...
            if ((UserMetalAdded > botWepAdded && botTicketAdded == 0 && change == false))
            {
            ...
            if (UserMetalAdded > ticketSell * botTicketAdded && botTicketAdded > 0 && change == false)
            {
        ...
        public int FindClosestDefindexInInventory(Inventory.Item[] inventory, string word)
        {
            ...
            foreach (Inventory.Item item in inventory)
            {
                if (inventorydict.ContainsKey(Trade.CurrentSchema.GetItem(item.Defindex).ItemName.ToLower()) == false && (Trade.CurrentSchema.GetItem(item.Defindex).CraftClass == "weapon" || Trade.CurrentSchema.GetItem(item.Defindex).CraftMaterialType == "weapon" || Trade.CurrentSchema.GetItem(item.Defindex).Defindex == 725))
                {
